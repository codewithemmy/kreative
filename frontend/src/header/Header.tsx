...
const Header: React.FC = () => {
  ...
  return (
    <nav className="w-full border mt-[4rem] md:mt-18 lg:mt-0 flex flex-col md:flex-row gap-3 md:justify-between py-2 px-2 lg:px-5 md:items-center">
      <div className="flex items-center justify-start gap-x-3">
        {parentTab ? (
          <h2 className="text-lg md:text-xl lg:text-2xl text-black p-2 md:p-8">
            {parentTab === "home" &&
              userDetails?.[0]?.name &&
              "Welcome, " + userDetails?.[0].name.split(" ")[0]}
          </h2>
        ) : (
          ""
        )}
