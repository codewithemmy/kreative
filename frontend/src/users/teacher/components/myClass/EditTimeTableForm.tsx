...
function EditTimeTableForm({ onClose, subjects, id, timeTableDay }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await ApiAuth.patch(`/time-table/${id}`, data, {
        headers: {
      ...
      setSent(!sent);
      setIsLoading(false);
      successNotifier("Time table updated successfully!");
    } catch (error) {
      setIsLoading(false);
      errorNotifier("Error updating time table");
    }
  };
