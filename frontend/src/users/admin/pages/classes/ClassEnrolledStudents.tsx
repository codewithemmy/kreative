...
function ClassEnrolledStudents() {
  ...
  const navigateToDetails = (id: string) => {
    navigate(`/admin/students/${id}`, {
      state: { studentList: studentList, classname: myClass?.[0]?.name, isTrue: isTrue },
    });
  };
    ...
    try {
      const response =
        adminAccess?.accountType === "admin"
          ? await ApiAuth.get(
              `/student?branchId=${adminDetails?.[0]?.branchId}&intendedClass=${classId}`,
              {
                headers: {
          ...
          : await ApiAuth.get(`/student?intendedClass=${classId}`, {
              headers: {
                Authorization: "Bearer " + adminAccess?.userToken,
              },
            });
...
export default ClassEnrolledStudents;
