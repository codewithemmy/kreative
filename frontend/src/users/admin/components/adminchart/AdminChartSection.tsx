...
const AdminChartSection: React.FC = () => {
  ...
  const [teachersChart, setTeachersChart] = useState<ChartData>({
    labels: [],
    datasets: [],
  });
  ...
  useEffect(() => {
    const handleGetTeachersAttendance = async () => {
      try {
        ...
        const response = await ApiAuth.get(
          `/attendance?period=${periodMap[timeFrame]}&branchId=${branchId}`,
          {
            headers: {
    ...
    handleGetTeachersAttendance();
  }, [adminAccess?.userToken, timeFrame, branchId]);
