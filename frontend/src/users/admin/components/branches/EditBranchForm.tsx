...
function EditBranchForm({ onClose, id }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await ApiAuth.patch(`/branch${id}`, filteredData, {
        headers: {
      ...
      setSent(!sent);
      setIsLoading(false);
      successNotifier("Branch updated successfully!");
    } catch (error) {
      setIsLoading(false);
      console.error("Error:", error);
      errorNotifier("Error updating branch");
    }
  };
        ...
        <div className="p-4">
          ...
          >
            ...
            <div className="flex justify-center items-center w-[100%]">
              <CustomButton
                text={isLoading ? "Updating..." : "Update Branch"}
                width="300px"
              />
            </div>
          </form>
